<template>
  <RichModal modal-id="edit-user" title="users.edit">
    <FormProvider @submit="updateUser" :form-model="formModel">
      <InputField
        :defaut-value="formModel.name"
        label-i18n="users.Name"
        name="name"
        :rules="[{ required: true }]"
      />
      <InputField
        :defaut-value="formModel.email"
        label-i18n="users.Email"
        name="email"
        :rules="[{ required: true }]"
      />
      <InputField
        :defaut-value="formModel.password"
        label-i18n="users.password"
        name="password"
        :rules="[{ required: true }]"
      />
      <CheckboxField
        v-for="r in ((roles as any)?.data || [])"
        :key="r"
        :name="r.id"
        :label-i18n="r.name"
      />
...
const { mutate: loadRoles, data: roles } = useService({
