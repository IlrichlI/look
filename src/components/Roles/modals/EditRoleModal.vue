<template>
  <RichModal modal-id="edit-role" title="roles.edit">
    <FormProvider @submit="createPermission">
      <InputField
        label-i18n="roles.Name"
        name="roleName"
        :defaut-value="((modals as any)['edit-role']?.payload as any).name"
      />
      <template
...
const { mutate: mutateEditRole, loading: loadingEditRole } = useService({
  serviceName: 'rolesService',
  actionName: 'update'
})
const { mutate: mutateGeneratePermission, loading: loadingGeneratePermission } = useService({
  serviceName: 'rolesService',
  actionName: 'generate'
})
const { closeModal, modals } = useModal()

const loadingSaveButton = computed(() => loadingEditRole.value || loadingGeneratePermission.value)
